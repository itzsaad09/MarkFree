...
async function removeWatermark(videoElement, regions) {
  return new Promise((resolve, reject) => {
    ...
    // Draw one frame to get video blob
    ctx.drawImage(videoElement, 0, 0, canvas.width, canvas.height);
    canvas.toBlob(async (thumbnailBlob) => {
      try {
        ...
        const uploadRes = await fetch('http://localhost:5000/remove-watermark', {
          method: 'POST',
          body: formData,
        });
        ...
        processingStatus.textContent = "Downloading clean video...";
        const cleanBlob = await uploadRes.blob();
